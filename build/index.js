...
const sendEmailUsingAPI = async (api_url, recipients, subject, body, isHtml) => {
...
const makeEmailNotifyBody = (recipients, description, timezoneDescription, startDateTime, endDateTime, meetingUrl) => {
    const html = `<html>
    <body>
    ...
    pool.query(sql, [], function (err, result) {
        if (err) {
            ...
            for (let i = 0; i < result.rows.length; i++) {
                ...
                console.log(id, description, participant_email_ids, timezone, timezone_description, start_time, end_time, organiser_id, notify_before_minutes);
                ...
                //   str_end_time
                // );
                let htmlBody = makeEmailNotifyBody(recipients, description, timezone_description, str_start_time, str_end_time, meetingUrl);
                let emailSubject = `Event Starting Soon:  + ${description.substring(0, 100)} ...`;
                ...
                where id=ANY($1);
                `;
                        pool.query(sql1, [meetingIdsNotified], function (err, result) {
                            pool.end(() => { });
                            if (err) {
                                console.log(err);
                            }
...
const makeMarketingEmailBody = (name, email) => {
    ...
    <h4>Thank You from <a href="https://scuoler.com">Scuoler</a> team<br/></h4>
    <a href="https://scuoler.com">https://scuoler.com</a>. 
    <h4><a href="https://scuoler.com/emailUnsubscribe?id=${accessToken}">Unsubscribe from mailing list</h4></p>
    </section>
    </body>
    </html>
   `;
    ...
    pool.query(sql, [], function (err, result) {
        if (err) {
            ...
            for (let i = 0; i < result.rows.length; i++) {
                ...
                where email=ANY($1 )
                `;
                        pool.query(sql1, [sent_emails], function (err, result) {
                            pool.end(() => { });
                            if (err) {
                                console.log(err);
                            }
