...
const makeMarketingEmailBody = (name: string, email: string) => {
...
const sendMarketingEmails = () => {
    ...
    pool.query(sql, [], function (err: Error, result: QueryResult<any>) {
        if (err) {
            ...
            const sent_emails: string[] = [];
            for (let i = 0; i < result.rows.length; i++) {
                let name: string = result.rows[i].first_name;
                let email: string = result.rows[i].email;
                ...
                where email=ANY($1 )
                `;
                            pool.query(sql1, [sent_emails], function (err: Error, result: QueryResult<any>) {
                                pool.end(() => { });
                                if (err) {
                                    console.log(err);
                                }
